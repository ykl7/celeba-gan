...
# check attributes and concatenate tensors along given axis
if "concat_v2" in dir(tf):
    def concatenate(tensors, axis, *args, **kwargs):
...
def concatenate_conditioning_vector(x, y):
    shapes_x = x.get_shape()
    shapes_y = y.get_shape()
    return concatenate([x, y*tf.ones([shapes_x[0], shapes_x[1], shapes_x[2], shapes_y[3]])], 3)
...
def linear(input_, output_size, scope=None, stddev=0.02, starting_bias=0.0, with_w=False):
    ...
    with tf.variable_scope(scope or "Linear"):
        ...
        bias = tf.get_variable("bias", [output_size], initializer=tf.constant_initializer(starting_bias))
        if with_w:
